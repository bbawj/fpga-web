...
class TB:
    def __init__(self, dut):
        ...
        self.sink = RgmiiSink(dut.phy_txd, dut.phy_txctl, dut.clk, dut.rst)
...
async def test_mac_mii_output(dut):
    ...
    tb.dut.mac_dest.value = 0x0000DEADBEEF;
    ...
    for test_data in test_frames:
        ...
        for d in test_frame.get_payload(strip_fcs=True):
            tb.dut.mac_phy_txd.value = BinaryValue(d & 0xF, 4, False, 0)
            await FallingEdge(tb.dut.clk)
            tb.dut.mac_phy_txd.value = BinaryValue((d >> 4) & 0xF, 4, False, 0)
            await RisingEdge(tb.dut.clk)
