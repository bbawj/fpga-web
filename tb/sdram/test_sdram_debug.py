...
async def sdram_ctrl(dut):
    tb = TB(dut)
    await tb.reset()
    await with_timeout(RisingEdge(dut.sdram_wr_granted), 400, "us")
    await Timer(50, "us")
    assert False
...
def test_sdram():
    ...
    runner.test(waves=True,
                verbose=True,
                hdl_toplevel="test_sdram_debug", test_module="test_sdram_debug")
